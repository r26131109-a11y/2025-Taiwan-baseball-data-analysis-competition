...
run_one_season <- function(dat, df, year = 2025) {
  ...
  # 從 joint_additive 的輸出抓「各隊最後 Elo」
...
### 做CV (把訓練集的月份的一個砍掉，最後取平均，選出該月該方法最好的模型)
...
path2025 <- file.path(desktop, "mlb_schedule_2025.xlsx")
...
path2024 <- file.path(desktop, "mlb_schedule_2024.xlsx")
...
path2023 <- file.path(desktop, "mlb_schedule_2023.xlsx")
...
path2022 <- file.path(desktop, "mlb_schedule_2022.xlsx")
...
path2021 <- file.path(desktop, "mlb_schedule_2021.xlsx")
...
elo_final2021 <- read_excel(file.path(desktop, "elo_final2021.xlsx"))%>%
  mutate(season = 2021)
elo_final2022 <- read_excel(file.path(desktop, "elo_final2022.xlsx"))%>%
  mutate(season = 2022)
elo_final2023 <- read_excel(file.path(desktop, "elo_final2023.xlsx"))%>%
  mutate(season = 2023)
elo_final2024 <- read_excel(file.path(desktop, "elo_final2024.xlsx"))%>%
  mutate(season = 2024)
elo_final2025 <- read_excel(file.path(desktop, "elo_final2025.xlsx"))%>%
  mutate(season = 2025)
...
# colley_2021 <- read_csv("C:/Users/hungh/OneDrive/桌面/2021_colley.csv")
# colley_2022 <- read_csv("C:/Users/hungh/OneDrive/桌面/2022_colley.csv")
# colley_2023 <- read_csv("C:/Users/hungh/OneDrive/桌面/2023_colley.csv")
# colley_2024 <- read_csv("C:/Users/hungh/OneDrive/桌面/2024_colley.csv")
# colley_2025 <- read_csv("C:/Users/hungh/OneDrive/桌面/2025_colley.csv")

colley_2021 <- read_csv(file.path(desktop, "2021_colley.csv"))
colley_2022 <- read_csv(file.path(desktop, "2022_colley.csv"))
colley_2023 <- read_csv(file.path(desktop, "2023_colley.csv"))
colley_2024 <- read_csv(file.path(desktop, "2024_colley.csv"))
colley_2025 <- read_csv(file.path(desktop, "2025_colley.csv"))

colley_2021$cut_date <- as.Date(colley_2021$cut_date)
colley_2022$cut_date <- as.Date(colley_2022$cut_date)
colley_2023$cut_date <- as.Date(colley_2023$cut_date)
colley_2024$cut_date <- as.Date(colley_2024$cut_date)
colley_2025$cut_date <- as.Date(colley_2025$cut_date)
elo_final2021$cut_date <- as.Date(elo_final2021$cut_date)
elo_final2022$cut_date <- as.Date(elo_final2022$cut_date)
elo_final2023$cut_date <- as.Date(elo_final2023$cut_date)
elo_final2024$cut_date <- as.Date(elo_final2024$cut_date)
elo_final2025$cut_date <- as.Date(elo_final2025$cut_date)
...
# 2025 要評估的五個 cutoff_date

month_end_cuts <- panel_all %>%
  filter(
    season == 2025,
    lubridate::day(cut_date) == 20,
    lubridate::month(cut_date) %in% 6:9
  ) %>%
  distinct(cut_date) %>%
  arrange(cut_date) %>%
  pull(cut_date)
